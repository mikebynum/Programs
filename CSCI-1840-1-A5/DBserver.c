...
/*                                                                    */
/* Usage: ./server                                                    */
/* used delete and insert from book                                   */
/*                                                                    */
/**********************************************************************/
...
void sendlist (ListNode *t, int sd, int recs); /*sends the list to the client*/
int compare ( srec *a, srec *b, int type); /* compare and sort the list by the type */
void insert(struct node **, srec *, int ); /* insert a new record in the list */
void delete( ListNodePtr *, int ); /* delete a record from the list */
...
main()
{
    ...
    op = fopen("Records", "rb"); /* open the file */
		if (op == NULL)
        printf ("There was no data to be read.\n");
    else 
	{
        printf ("'Records' successfully read.\n");
 
        while (fread(&newrec, sizeof (srec), 1, op) != 0) /* read each record in */
		{
	...
	while ( command ) /*keep receiving until the command was entered*/
	{
		switch(command)
		{
			...
			case stpcmd:
				...
				fclose(fp);
				printf("Now exiting server.\n");
				exit(0);
			default:
				printf("Correct command was not given!");
				break;
		}
